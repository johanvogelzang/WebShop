/**
 * DO NOT MODIFY THIS FILE, CHANGES WILL BE AUTOMATICALLY OVERWRITTEN! 
 *
 * Generated by Mod4j. 
 *     Time             : Fri Jun 10 13:47:09 CEST 2011
 *     Application model: Order
 *     Generator        : Mod4j BusinessDomainDsl generator: DaoInterfaceBase.xpt
 */

package org.company.recordshop.data;

import java.util.List;

import org.company.recordshop.domain.Order;
import org.company.recordshop.domain.OrderExample;

/**
 * OrderDaoBaseData Access Object (DAO). Responsible for accessing database.
 * 
 * @author Mod4j generated
 * 
 */
public interface OrderDaoBase {

	/**
	 * Retrieves a Order by its ID from the database.
	 * 
	 * @param id id of the Order to retrieve.
	 * @return Order retrieved or null when no Order was found.
	 */
	Order retrieve(Long id);

	/**
	 * Adds a new Order to the database
	 * 
	 * @param object
	 *                the Order to add to the database.
	 * @return the stored Order.
	 */
	Long add(Order object);

	/**
	 * Deletes a Order from the database.
	 * 
	 * @param object
	 *                the Order to delete
	 */
	void delete(Order object);

	/**
	 * Selects all Orders from the database.
	 * 
	 * @return a List of all Order objects.
	 */
	List<Order> listAll();

	/**
	 * Selects a range (page) of Orders from the database.
	 * 
	 * @param firstResult the index of the first result object to be retrieved (numbered from 0)
	 * @param maxResults  the maximum number of result objects to retrieve (or <=0 for no limit) 
	 * 
	 * @return a List containing 0 or more Order objects.
	 *
	 * @see #listPage(int, int)
	 */
	List<Order> listPage(final int firstResult, final int maxResults);

	/**
	 * Selects a sorted range (page) of Orders from the database.
	 * 
	 * @param firstResult the index of the first result object to be retrieved (numbered from 0)
	 * @param maxResults the maximum number of result objects to retrieve (or <=0 for no limit)
	 * @param sortProperty the property of the object to sort on
	 * @param isAscending should the retruned list be sorted ascending? 
	 * 
	 * @return a sorted List containing 0 or more Order objects.
	 *
	 * @see #listPage(int, int, String, boolean)
	 */
	List<Order> listPage(final int firstResult, final int maxResults,
			final String sortProperty, final boolean isAscending);

	/**
	 * Selects a sorted range (page) of Orders from the database.
	 * 
	 * @param example the example object to which the result objects need to conform
	 * @param firstResult the index of the first result object to be retrieved (numbered from 0)
	 * @param maxResults the maximum number of result objects to retrieve (or <=0 for no limit)
	 * @param sortProperty the property of the object to sort on
	 * @param isAscending should the retruned list be sorted ascending? 
	 * 
	 * @return a sorted List containing 0 or more Order objects.
	 *
	 * @see #listPage(int, int, String, boolean)
	 */
	List<Order> listPageByExample(final OrderExample example,
			final int firstResult, final int maxResults,
			final String sortProperty, final boolean isAscending);

	/**
	 * Counts the number of persisted Order objects in the database.
	 * 
	 * @return the number of persisted objects in the database.
	 */
	long count();

	/**
	 * Counts the number of persisted Order objects in the database that
	 * matches the given example object.
	 *
	 * @param example
	 *                the example object to be counted
	 * 
	 * @return the number of persisted objects in the database.
	 */
	long count(final OrderExample example);

	/**
	 * Selects Orders from the database that matches an
	 * example object using Query-by-Example(QBE).
	 * 
	 * @param example
	 *                the example object to which the result objects need to conform
	 * @return a List of Order objects.
	 */
	List<Order> findByExample(OrderExample example);
}
