...
import org.company.webshop.data.KaderDaoBase;
...
/**
 * Kader Data Access Object (DAO) Implementation. Responsible for Creating,
 * Reading, Updating, Deleting and retrieving lists of Kader objects in the database.
 * All database access is done here.
 *  
 * @author Generated by Mod4j
 */
public class KaderDaoImplBase extends HibernateDaoSupport
		implements
			KaderDaoBase {
	 ...
	 * {@inheritDoc}
	 */
	public Kader retrieve(Long id) {
		Object o = getHibernateTemplate().get(Kader.class, id);
		if (o == null) {
			return null;
		}
		return (Kader) o;
	 ...
	 * @return Kader
	 **/
	public void update(Kader object) {
	 ...
	 * {@inheritDoc}
	 */
	public void delete(Kader object) {
	...
	public List<Kader> listAll() {
		return getHibernateTemplate().find("from Kader");
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Kader> listPage(final int firstResult, final int maxResults) {

		DetachedCriteria criteria = DetachedCriteria.forClass(Kader.class);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Kader> listPageByExample(final KaderExample example,
		...
		result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(final Session session)
				...
				return session.createCriteria(Kader.class).add(
	...
	public long count() {
		return ((Long) getHibernateTemplate().iterate(
				"select count(*) from Kader").next()).longValue();
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long count(final KaderExample example) {
		Assert.notNull(example, "argument [example] may not be null");
		List<Integer> result;
		result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(final Session session)
					throws HibernateException, SQLException {
				Criteria crit = session.createCriteria(Kader.class);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Kader> findByExample(final KaderExample example) {
		Assert.notNull(example, "argument [example] may not be null");
		List<Kader> result;
		result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(final Session session)
